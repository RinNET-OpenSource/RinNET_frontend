...
function RatingRecord({ item, index }: { item: ChuniV2RatingItem; index: number }) {
  const level = difficulty(item.level);
  return (
    <div className="col-12 col-md-6 col-xxl-4">
      <div className="card rating-card card-btn">
        ...
        <>
          ...
          <div className="row mb-4 g-2">
            {newRating.map((item, index) => (
              <RatingRecord item={item} index={index} key={`${item.musicId}-${item.level}-${index}`} />
            ))}
          </div>
        </>
      )}
